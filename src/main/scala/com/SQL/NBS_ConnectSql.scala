...
object NBS_ConnectSql {
  def apply(dateType: String, dataAr: Array[String]) = dateType match {
    case "月度数据" => {
      ...
      val str = s"insert into nbs_month (dateType,tag1,tag2,tag3,date,data)values('" + NationalData_Worker2.dataType + "','" + dataAr(0) + "','" + dataAr(1) + "','" + dataAr(2) + "','" + dataAr(3) + "','" + dataAr(4) + "')"
      nc.insetData(str)
    ...
    case "分省月度数据" => {
      val nc = new NBS_ConnectSql
      val str = s"insert into nbs_otherData (dateType,tag1,tag2,tag3,date,data)values('" + NationalData_WkOther.dataType + "','" + dataAr(0) + "','" + dataAr(1) + "','" + dataAr(2) + "','" + dataAr(3) + "','" + dataAr(4) + "','" + dataAr(5) + "')"
      nc.insetData(str)
